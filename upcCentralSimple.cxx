...
struct CentralBarrelAnalyzerSimple {
  ...
  HistogramRegistry registry{
    ...
    {{"hEffectOfSelections", "Effect of cuts;Selection (-);Number of events (-)", {HistType::kTH1D, {{20, -0.5, 19.5}}}},
     ...
     {"hInvariantMassWide", ";Invariant mass (GeV/c^{2});Number of events (-)", {HistType::kTH1D, {{1000, 0., 5.}}}},
     {"hInvariantMassElEl", ";Invariant mass (GeV/c^{2});Number of events (-)", {HistType::kTH1D, {{1000, 0., 5.}}}},
     {"hInvariantMassMuMu", ";Invariant mass (GeV/c^{2});Number of events (-)", {HistType::kTH1D, {{1000, 0., 5.}}}},
     {"hInvariantMassPiPi", ";Invariant mass (GeV/c^{2});Number of events (-)", {HistType::kTH1D, {{1000, 0., 5.}}}},
     {"hInvariantMassKaKa", ";Invariant mass (GeV/c^{2});Number of events (-)", {HistType::kTH1D, {{1000, 0., 5.}}}},
     {"hInvariantMassPrPr", ";Invariant mass (GeV/c^{2});Number of events (-)", {HistType::kTH1D, {{1000, 0., 5.}}}},
     ...
     {"hTPCsignalMuons", ";Identified muon 1 - TPC d#it{E}/d#it{x} (arb. units);Identified muon 2 - TPC d#it{E}/d#it{x} (arb. units)", {HistType::kTH2F, {{950, 10., 200.}, {950, 10., 200.}}}},
     {"hTPCsignalPions", ";Identified pion 1 - TPC d#it{E}/d#it{x} (arb. units);Identified pion 2 - TPC d#it{E}/d#it{x} (arb. units)", {HistType::kTH2F, {{950, 10., 200.}, {950, 10., 200.}}}},
     {"hTPCsignalKaons", ";Identified kaon 1 - TPC d#it{E}/d#it{x} (arb. units);Identified kaon 2 - TPC d#it{E}/d#it{x} (arb. units)", {HistType::kTH2F, {{950, 10., 200.}, {950, 10., 200.}}}},
     {"hTPCsignalProtons", ";Identified proton 1 - TPC d#it{E}/d#it{x} (arb. units);Identified proton 2 - TPC d#it{E}/d#it{x} (arb. units)", {HistType::kTH2F, {{950, 10., 200.}, {950, 10., 200.}}}},
  ...
  using LimitedUDTracks = soa::Join<aod::UDTracks, aod::UDTracksExtra, aod::UDTracksPID>;
  using LimitedUDCollision = aod::UDCollisions::iterator;
  // using LimitedUDCollision = soa::Join<aod::UDCollisions, aod::UDCollisionsSels>::iterator;
  ...
  // process
  void processCentralBarrel(LimitedUDCollision const& collision, LimitedUDTracks const& tracks)
  ...
  //                           soa::Join<aod::Collisions, aod::EvSels> const& infoEventSelection)
  {
    ...
    if (verboseInfo)
      printLargeMessage("NEW COLLISION");
    countCollisions++;
    ...
    // if (verboseDebug)
    //   LOGF(info, "collision number = %d has %d contributors", countCollisions, collision.numContrib());
    ...
    for (auto& track : tracks) {
      if (!selectTrack(track, applyTrackCuts))
        continue;
      if (trackSelection(track, 2))
        countContributorsToVertex++;
      if (selectTrack(track, 2))
        countGlobalContributorsToVertex++;
      // if (isUDprimaryTrack(track))
      //   countPrimaryTracks++;
      if (track.hasTPC()) {
    ...
    if (countGlobalContributorsToVertex == 2)
      registry.get<TH1>(HIST("hEffectOfSelections"))->Fill(4);
    ...
    if (countContributorsToVertex == 2) {
      ...
      for (auto& track : tracks) {
        ...
        trackFromVertex.SetPxPyPzE(track.px(), track.py(), track.pz(),
                                   energy(constants::physics::MassPionCharged, track.px(), track.py(), track.pz()));
        sumOfVertexTracks += trackFromVertex;
        if (track.hasTPC()) {
          if (countTrks == 1)
            dedxVC[0] = track.tpcSignal();
          else if (countTrks == 2)
            dedxVC[1] = track.tpcSignal();
          else
      ...
      if (isTwinParticles) {
        if (twinParticles == P_ELECTRON)
          registry.get<TH1>(HIST("hInvariantMassElEl"))->Fill(sumOfVertexTracks.M());
        if (twinParticles == P_MUON)
          registry.get<TH1>(HIST("hInvariantMassMuMu"))->Fill(sumOfVertexTracks.M());
        if (twinParticles == P_PION)
          registry.get<TH1>(HIST("hInvariantMassPiPi"))->Fill(sumOfVertexTracks.M());
        if (twinParticles == P_KAON)
          registry.get<TH1>(HIST("hInvariantMassKaKa"))->Fill(sumOfVertexTracks.M());
        if (twinParticles == P_PROTON)
          registry.get<TH1>(HIST("hInvariantMassPrPr"))->Fill(sumOfVertexTracks.M());
      }
    }

  } // end processCentralBarrel

  PROCESS_SWITCH(CentralBarrelAnalyzerSimple, processCentralBarrel, "Iterate central barrel data using UD tables", true);
};

WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  return WorkflowSpec{
    adaptAnalysisTask<CentralBarrelAnalyzerSimple>(cfgc, TaskName{"upc-central-barrel-simple"})};
}