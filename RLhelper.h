...
{
  if (generatedElectron.pdgCode() == 11)
    return -1;
  else if (generatedElectron.pdgCode() == -11)
    return 1;
  else
    return 0;
}
...
// reads full event selection table end return global index corresponding to given global BC. Return -1 when fails.
{
  for (auto& infoEvSel : infoEvSels) {
    if (infoEvSel.bcId() == globalBC)
      return infoEvSel.globalIndex();
  }
  return -1;
}
...
{
  if (collision.has_foundFT0() || collision.has_foundFV0() || collision.has_foundFDD())
    return false;
  else
    return true;
}
...
//   if (hasNoFT0 && hasNoFV0A && hasNoFDD)
//     return true;
//   else
//     return false;
// }
...
//   // selection DCAxy
//   if (abs(udtrack.dcaXY()) > maxDcaXY)
//     return false;
//   // selection DCAz
//   if (abs(udtrack.dcaZ()) > maxDcaZ)
//     return false;
...
// TrackSelection::kInAcceptanceTracks = kPtRange | kEtaRange;
{
  ...
  // selection pt
  if (udtrack.pt() < minPt || udtrack.pt() > maxPt)
    return false;
  // selection eta
  if (currentEta < minEta || currentEta > maxEta)
    return false;
  // passed all selections
  return true;
}
...
// TrackSelection::kQualityTracks = kTrackType | kTPCNCls | kTPCCrossedRows | kTPCCrossedRowsOverNCls | kTPCChi2NDF | kTPCRefit | kITSNCls | kITSChi2NDF | kITSRefit | kITSHits;
{
  ...
  if (!udtrack.hasITS())
    return false;
  // ITS chi2/ndf
  if (udtrack.itsChi2NCl() > cutITSchi2ndf)
    return false;
  // ITS n clusters
  if (udtrack.itsNCls() < cutNitsClusters)
    return false;
  // TPC refit
  if (!udtrack.hasTPC())
    return false;
  // TPC chi2/ndf
  if (udtrack.tpcChi2NCl() > cutTPCchi2ndf)
    return false;
  // TPC n clusters
  int16_t nFoundClusters = udtrack.tpcNClsFindable() - udtrack.tpcNClsFindableMinusFound();
  if (nFoundClusters < cutNtpcClusters)
    return false;
  // TPC crossed rows
...
{
  if (!isUDinAcceptanceTrack(udtrack))
    return false;
  // if (!isUDprimaryTrack(udtrack))
  //   return false;
  if (!isUDqualityTrack(udtrack))
    return false;
  return true;
}
...
// Do selection of reconstructed track
{
  ...
  // if (selection == 2 && udtrack.isPVContributor() != 1)
  //   return false;
  // Is central barrel track selection global track
  if (selection == 3 && isUDqualityTrack(udtrack) != 1)
    return false;
  // Is central barrel track selection global track
  // if (selection == 4 && isUDprimaryTrack(udtrack) != 1)
  //   return false;
  // Is central barrel track selection global track
  if (selection == 5 && isUDglobalTrack(udtrack) != 1)
    return false;
...
// Do selection of reconstructed track
{
  if (setOfCuts < 1)
    return true;
  // Is central barrel propagated track)
  if (setOfCuts < 2 && trackSelection(track, 1) != 1)
    return false;
  // Is central barrel vertex contributor
  if (setOfCuts < 3 && trackSelection(track, 2) != 1)
    return false;
...
// Fill into histogram information from EventSelection task for each collision
{
  ...
  for (int i = 0; i < kNsel; i++) {
    if (collision.selection()[i])
      registry.get<TH1>(HIST("hEventSelectionTaskParameters"))->Fill(i);
  }
...
// Fill into histogram effect of track selection for all tracks
{
  ...
  for (auto& track : tracks) {
    if (trackSelection(track, 0))
      registry.get<TH1>(HIST("hEffectOfTrackSelections"))->Fill(0);
    registry.get<TH1>(HIST("hEffectOfTrackSelections"))->GetXaxis()->ChangeLabel(1, 70, 0.02, 30, -1, -1, "no cut");
    if (trackSelection(track, 1))
      registry.get<TH1>(HIST("hEffectOfTrackSelections"))->Fill(1);
    registry.get<TH1>(HIST("hEffectOfTrackSelections"))->GetXaxis()->ChangeLabel(2, 70, 0.02, 30, -1, -1, "propagated");
    if (trackSelection(track, 3))
      registry.get<TH1>(HIST("hEffectOfTrackSelections"))->Fill(2);
    registry.get<TH1>(HIST("hEffectOfTrackSelections"))->GetXaxis()->ChangeLabel(3, 70, 0.02, 30, -1, -1, "quality");
    if (trackSelection(track, 4))
      registry.get<TH1>(HIST("hEffectOfTrackSelections"))->Fill(3);
    registry.get<TH1>(HIST("hEffectOfTrackSelections"))->GetXaxis()->ChangeLabel(4, 70, 0.02, 30, -1, -1, "primary");
    if (trackSelection(track, 5))
      registry.get<TH1>(HIST("hEffectOfTrackSelections"))->Fill(4);
    registry.get<TH1>(HIST("hEffectOfTrackSelections"))->GetXaxis()->ChangeLabel(5, 70, 0.02, 30, -1, -1, "global");
    if (trackSelection(track, 2))
      registry.get<TH1>(HIST("hEffectOfTrackSelections"))->Fill(5);
...
// Function to loop over tracks associated to a collision and print basic info
{
  ...
  for (auto& track : tracks) {
    if (!selectTrack(track, setOfCuts))
      continue;
    if (track.has_udMcParticle()) {
